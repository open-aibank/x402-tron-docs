...
由您自行运行。x402-tron 采用自托管设计，代码仓库中提供的促进者程序已准备就绪，可直接运行。
...
* **上限计费 (Upto) 模式**：用户授权最高支付限额，最终成本取决于实际使用量。

#### x402-tron 支持哪些支付方案？

x402-tron 目前支持 `upto` (上限计费) 方案，其核心机制允许：
* 客户端授权一个**最高支付金额**。
...
`upto` 方案属于**推送支付 (Push Payment)**——交易一旦上链执行即不可逆转。处理退款通常有以下两种方式：

1.  **业务层退款：** 由卖方主动发起一笔新的 USDT 转账，将资金返还给买方。
2.  **按实结算（预防性）：** 利用 `upto` 方案特性，服务端仅结算实际产生的费用，而非全额扣款（从而避免需要退款的情况）。
